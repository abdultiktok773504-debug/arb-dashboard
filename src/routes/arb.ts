...
import { executeTrade, tradeHistory } from "../lib/trader";
import { getMexcBalance } from "../lib/mexc";
import { getWeexBalance } from "../lib/weex";
...
const session = {
  ...
  topPerformers: Object.fromEntries(
    SYMBOLS.map((s) => [s, { bestSpreadPercent: 0, opportunityCount: 0, estimatedTotalProfit: 0 }])
  ) as Record<string, { bestSpreadPercent: number; opportunityCount: number; estimatedTotalProfit: number }>,
};
...
async function fetchWeexPrice(symbol: string): Promise<number | null> {
  try {
    const res = await fetch(`https://api.bitget.com/api/v2/spot/market/tickers?symbol=${symbol}USDT`,
      { signal: AbortSignal.timeout(5000) });
    if (!res.ok) return null;
    const data = await res.json() as { data?: Array<{ lastPr?: string; close?: string }> };
...
async function scanPrices() {
  ...
  for (const { sym, mexc, weex } of results) {
    ...
    if (spreadPercent > MIN_SPREAD_PERCENT) {
      ...
      if (spreadPercent > session.bestSpread) { session.bestSpread = spreadPercent; session.bestSymbol = `${sym}/USDT`; }
      const tp = session.topPerformers[sym];
      if (tp) {
        if (spreadPercent > tp.bestSpreadPercent) tp.bestSpreadPercent = spreadPercent;
...
let lastScan: { prices: unknown[]; opportunities: unknown[]; ts: number } | null = null;
...
const autobot = { enabled: false, tradesExecuted: 0, lastTradeAt: null as string | null, timer: null as ReturnType<typeof setTimeout> | null };

async function autobotTick() {
  if (!autobot.enabled) return;
  try {
    ...
    if (!canBuyMexc && !canBuyWeex) {
      ...
      if (autobot.enabled) autobot.timer = setTimeout(() => { void autobotTick(); }, 10000);
      return;
    }

    const scan = await getCachedScan();
    const opps = scan.opportunities as Array<{ symbol: string; buyExchange: string; sellExchange: string; buyPrice: number; sellPrice: number; spreadPercent: number }>;
    for (const opp of opps) {
      if (!autobot.enabled) break;
      if (opp.buyExchange === "MEXC" && !canBuyMexc) continue;
      if (opp.buyExchange === "WEEX" && !canBuyWeex) continue;
      console.log(`[BOT] Trading ${opp.symbol} spread=${opp.spreadPercent.toFixed(3)}%`);
      const result = await executeTrade(opp.symbol, opp.buyExchange, opp.sellExchange, opp.buyPrice, opp.sellPrice, opp.spreadPercent, BUDGET_PER_TRADE);
      if (result.status === "success" || result.status === "partial") {
        autobot.tradesExecuted += 1;
        autobot.lastTradeAt = new Date().toISOString();
      }
    }
  } catch (err) { console.error("[BOT] Tick failed:", err); }
  if (autobot.enabled) autobot.timer = setTimeout(() => { void autobotTick(); }, 3000);
}

function startAutobot() {
  if (autobot.timer) clearTimeout(autobot.timer);
  autobot.timer = setTimeout(() => { void autobotTick(); }, 1000);
  console.log("[BOT] Started");
}

function stopAutobot() {
  if (autobot.timer) { clearTimeout(autobot.timer); autobot.timer = null; }
...
router.post("/arb/trade", async (req, res) => {
  const { symbol, buyExchange, sellExchange, buyPrice, sellPrice, spreadPercent } = req.body as {
    symbol: string; buyExchange: string; sellExchange: string;
    buyPrice: number; sellPrice: number; spreadPercent: number;
  };
  if (!symbol || !buyExchange || !sellExchange) {
    res.status(400).json({ error: "Missing required fields" });
    return;
  }
  const result = await executeTrade(symbol, buyExchange, sellExchange, buyPrice, sellPrice, spreadPercent, BUDGET_PER_TRADE);
  res.json(result);
});
...
router.get("/arb/balances", async (_req, res) => {
  const [mexcUsdt, weexUsdt] = await Promise.all([getMexcBalance("USDT"), getWeexBalance("USDT")]);
  const mexcCoins: Record<string, number> = {};
  const weexCoins: Record<string, number> = {};
  await Promise.all(SYMBOLS.map(async (sym) => {
    const [m, w] = await Promise.all([getMexcBalance(sym), getWeexBalance(sym)]);
    if (m > 0) mexcCoins[sym] = m;
    if (w > 0) weexCoins[sym] = w;
  }));
